...
void ClientSession::run(http::verb verb, const std::string& host, const std::string& port, const std::string& target,
                        const std::string& body, int version, const std::map<std::string, std::string>& fields)
{
    ...
    resolver_.async_resolve(host, port, beast::bind_front_handler(&ClientSession::on_resolve, shared_from_this()));
}
...
void ClientSession::on_resolve(beast::error_code ec, tcp::resolver::results_type results)
{
    ...
    stream_.async_connect(results, beast::bind_front_handler(&ClientSession::on_connect, shared_from_this()));
}
...
void ClientSession::on_connect(beast::error_code ec, tcp::resolver::results_type::endpoint_type)
{
    ...
    http::async_write(stream_, req_, beast::bind_front_handler(&ClientSession::on_write, shared_from_this()));
}
...
void ClientSession::on_write(beast::error_code ec, std::size_t bytes_transferred)
{
    ...
    http::async_read(stream_, buffer_, res_, beast::bind_front_handler(&ClientSession::on_read, shared_from_this()));
}
...
std::future<http::response<http::string_body>> ClientSession::getResponse() { return finished_promise.get_future(); }
